...
from genesis.spine import iter_truth_paths, read_artifact_log
...
def immutable_artifacts() -> dict[str, Any]:
    ...
    for row in rows:
        ...
        if not bool(row.get("immutable", True)):
            duplicates[artifact_id or ""] = duplicates.get(artifact_id or "", 0) + 1
...
__all__ = ["append_only_logs", "enforce", "immutable_artifacts", "lineage_survival_floor", "replay_determinism"]
