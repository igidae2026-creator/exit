...
def pressure(metrics: Mapping[str, float]) -> dict[str, float]:
    ...
    p = {
        ...
        "diversity_pressure": max(0.0, 1.0 - float(metrics.get("diversity", 0.0))) + max(0.0, lc - 0.45),
    ...
    if lc > 0.45:
        p["diversity_pressure"] += 0.2
        p["reframing_pressure"] += 0.3
    return {key: round(float(value), 4) for key, value in p.items()}
