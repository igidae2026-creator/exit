...
def material_from_source(source: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "material_id": str(source.get("material_id") or "ops_runbook:source"),
        ...
        "metadata": {"project_type": "ops_runbook"},
    }


def artifact_from_output(output: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "artifact_id": str(output.get("artifact_id") or "ops_runbook:artifact"),
