...
def material_from_source(source: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "material_id": str(source.get("material_id") or "code:unknown"),
        ...
        "metadata": {"language": source.get("language", "python")},
    }


def artifact_from_output(output: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "artifact_id": str(output.get("artifact_id") or "code-artifact:unknown"),
        ...
        "metadata": {"files_touched": int(output.get("files_touched", 0) or 0)},
    }
