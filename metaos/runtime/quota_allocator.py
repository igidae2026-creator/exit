...
from metaos.runtime.hysteresis import bounded_step
...
def allocate(
    ...
    *,
    ...
    guard = dict(guard or {})
    base = max(2 if guard.get("downshift_workers") else 4, int(workers))
    ...
    target_workers = max(2 if guard.get("downshift_workers") else 4, int(base * scale))
    ...
    if not guard.get("downshift_workers") and exploratory_not_critical:
        target_workers = int(_clamp(float(target_workers), 12.0, 28.0))
        if prev_workers is not None and prev_workers >= 28:
            target_workers = min(target_workers, 26 + int(up >= 1.5))
    elif not guard.get("downshift_workers") and not strong_combined_pressure and down < 0.9:
        target_workers = int(_clamp(float(target_workers), 10.0, 28.0))
    if prev_workers is not None:
        ...
        hi = max(float(target_workers), float(prev_workers), 4.0)
        ...
        effective_workers = int(
            round(
                bounded_step(
                    ...
                    max_delta=max_delta,
                    lo=2.0 if guard.get("downshift_workers") else 4.0,
                    hi=hi,
                )
            )
        )
    ...
    if not strong_combined_pressure and not guard.get("downshift_workers"):
        effective_workers = int(_clamp(float(effective_workers), 10.0, 28.0))
