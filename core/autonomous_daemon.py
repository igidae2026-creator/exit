...
from core.quest import ensure_quest, update_quest
...
def worker(task: tuple[dict[str, float], list[str], int]) -> dict[str, Any]:
    ...
    try:
        ...
        if llm_fn is not None and domain_name:
            try:
                context = {"tick": tick, "domain": domain_name, "base_strategy": base_strategy}
        ...
        if strategy is None and domain_name:
            generator = resolve_domain_generate(domain_name)
            if generator is not None:
                try:
                    strategy = normalize_strategy(generator())
                    if strategy is not None:
